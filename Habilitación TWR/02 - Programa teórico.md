---
tags:
  - habilitacion-twr
  - teoria
---

# Programa teórico

Volver a [[00 - Inicio]]. Fuente principal: manual ATS-DATS-12, pp. 5–9.

## 1. Aeródromo, características físicas y ayudas visuales

- [x] Orientación, longitud y ancho de pistas.
- [x] Elevación de cabeceras.
- [x] Resistencia y limitaciones de uso.
- [x] Ubicación y designación de calles de rodaje.
- [x] Posiciones de estacionamiento y responsabilidad sobre ellas.
- [x] Posición prevista ante amenaza de bomba o interferencia ilícita.
- [x] Ayudas visuales y señalamiento de obstáculos.

Preguntas guía: ¿qué limitaciones cambian una autorización?, ¿dónde puede surgir una incursión?, ¿qué información debe recibir una aeronave antes de moverse?

## 2. Estructura del espacio aéreo

- [ ] Clasificación aplicable.
- [ ] Límites laterales y verticales de ATZ, CTR y TMA.
- [ ] Servicios suministrados y limitaciones.
- [ ] Circuito civil.
- [ ] Circuito militar.
- [ ] Circuito de helicópteros.

Producto de estudio: dibujar el espacio aéreo sin mirar y luego corregirlo contra la publicación vigente.

## 3. Reglas, procedimientos y fuentes

- [ ] POI entre TWR y APP Carrasco o ACC Montevideo.
- [ ] Coordinación con COA.
- [ ] Cartas de acuerdo con aeródromos privados.
- [ ] Doc. 4444: capítulos 3, 6, 7 y 15.
- [ ] LAR 211 y LAR 091.
- [ ] AIP Uruguay.
- [ ] Circulares operativas y administrativas.
- [ ] MADOR.

> [!warning]
> Registrar siempre edición, fecha de vigencia y enmiendas. Una nota de estudio antigua no reemplaza una fuente operacional vigente.

## 4. Instalaciones y servicios de navegación aérea

- [ ] Radioayudas: ubicación, frecuencia, uso y limitaciones.
- [ ] Cartas de aproximación.
- [ ] Aproximaciones de entrenamiento.
- [ ] SID y STAR aplicables.
- [ ] Estado de radioayudas y forma de verificarlo.

## 5. Equipos de control y utilización

- [ ] SDD: configuración, funciones, FPL y etiqueta.
- [ ] SDC: telefonía y comunicaciones.
- [ ] Luces de pista y calles de rodaje.
- [ ] Comunicaciones de emergencia.
- [ ] Reflector de señales.
- [ ] Monitores de estado de radioayudas.
- [ ] Estación meteorológica automática.
- [ ] Instrumentos analógicos alternativos.

Para cada equipo anotar: uso normal, verificación, limitaciones, indicación de falla, medio alternativo y coordinación requerida.

## 5 bis. Fraseología y radiotelefonía

- [ ] Estudiar [[08 - Fraseología TWR español-inglés]].
- [ ] Practicar secuencias normales de salida, circuito y llegada en ambos idiomas.
- [ ] Practicar colaciones, correcciones, incursión de pista, motor y al aire y emergencias.
- [ ] Confirmar fraseología y procedimientos locales con el OJTI.

## 6. Terreno y referencias

- [ ] Obstáculos: ubicación y señalamiento.
- [ ] Ayudas visuales terrestres.
- [ ] Referencias geográficas útiles para reportes y tránsito VFR.

## 7. Características del tránsito

- [ ] Vuelos locales.
- [ ] VMO.
- [ ] Vuelos simulados.
- [ ] VFR especiales.
- [ ] Circuitos según tipo y performance de aeronave.
- [ ] Operación de helicópteros.

## 8. Meteorología

- [ ] Nubes, niebla y bruma.
- [ ] Techo y visibilidad; determinación de visibilidad en pista.
- [ ] Viento y cortante de viento.
- [ ] Turbulencia y engelamiento.
- [ ] QNH, QFE y QNE.
- [ ] METAR, TAF, SIGMET y SPECI.
- [ ] Transmisión y recepción de información meteorológica.

## 9. Emergencias y búsqueda y salvamento

- [ ] Plan de emergencia del aeródromo.
- [ ] Alertas y cadena de notificación.
- [ ] Emergencias e interferencia ilícita.
- [ ] Contingencias y degradación CNS.

## 10. Dimensión actitudinal

- [ ] Relación con instructor y equipo.
- [ ] Comunicación interpersonal.
- [ ] Respuesta a observaciones y correcciones.
- [ ] Responsabilidad, puntualidad, compromiso y motivación.
- [ ] Conciencia situacional y toma de decisiones.
- [ ] Prevención, detección y recuperación del error.
- [ ] Uso de recursos y solicitud de apoyo.
- [ ] Trabajo en equipo, liderazgo y CRM.
- [ ] Estrés y fatiga.
